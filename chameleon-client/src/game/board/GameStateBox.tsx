...
export function PlayerTurnSpinner() {
    return <div role="status">
        <svg className="inline mr-2 w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-black"
...
export function GameStateBox({
    ...
    return <div className="flex mt-2 w-full">
        ...
        <div className="ml-2 w-8/12">
            <table>
                <tbody>
                {Object.entries(playerMap).map(([playerID, playerData]) => <PlayerToWord playerName={playerData.name}
                                                                                         playerRole={playerView.player_who_is_chameleon && playerID === playerView.player_who_is_chameleon ? "Chameleon" : "Unknown"}
                                                                                         word={playerView.player_words[playerID]}
